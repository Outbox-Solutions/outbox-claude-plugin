...
The `outbox-workflows` skill carries the shape of everything a workflow can
hold — trigger filters, the schedule trigger's cadence and target, the branch
and split actions, what an if/else branch may test, and why a call chase should
almost never be a chain of if_else blocks. Follow it rather than inferring from
the contract alone.
